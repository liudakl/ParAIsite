...
dataset_name = 'L96'
try:
    os.remove('sample-%s.ckpt'%(dataset_name))
except FileNotFoundError:
    pass
SetToUse, structure = return_dataset (dataset_name)
...
checkpoint_callback = ModelCheckpoint(monitor='val_Total_Loss',dirpath='best_models/',filename='sample-%s'%(dataset_name))
...
for nRuns in range (1,maxRuns+1):
    ...
    with open('best_models/val_idx_%s.pkl'%(nRuns), 'wb') as f:
...
#####   Plot Results  #######
    ...
    if min_mape_val < best_mape:
        ...
        #trainer.save_checkpoint("best_models/model_%s."%(dataset_name)+str(nRuns)+".pt")
        #lit_module.model.save("best_models/model_%s."%(dataset_name)+str(nRuns)+".pt")
        torch.save(lit_module.model, "best_models/model_full_lit_%s."%(dataset_name)+str(nRuns)+".pt")
        torch.save(trainer.model.model, "best_models/model_full_trainer_%s."%(dataset_name)+str(nRuns)+".pt")
