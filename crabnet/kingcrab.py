...
class CrabNet(nn.Module):
    ...
    def forward(self, src, frac):
        output = self.encoder(src, frac) # we took the last output layer and put as an input to the mlp model 
...
class combined_models (nn.Module): 
	def __init__(self, pretrained_model,MLP,out_dims=3,
                 d_model=512,
                 N=3,
                 heads=4,compute_device=None,residual_nn='roost'):
		...
		self.heads = heads
		self.compute_device = compute_device
		self.pretrained = pretrained_model #nn.Sequential(*list(pretrained_model.children())[:-1])  # Here I removed the last layer 
		self.MLP = MLP 
	...
	def forward2(self, src, frac):
		output = self.pretrained.encoder(src, frac)

		mask = (src == 0).unsqueeze(-1).repeat(1, 1, 128) #self.out_dims)
