...
from utils import (Lamb, Lookahead, RobustL1, BCEWithLogitsLoss,
                         EDM_CsvLoader, Scaler, DummyScaler, count_parameters, ownStandardScaler)
from get_compute_device import get_compute_device
from optim import SWA
...
class Model():
    ...
    def load_data(self, file_name, batch_size=2**9, train=False):
        ...
        if train:
            ...
            if self.classification:
                ...
                self.scaler = ownStandardScaler(y) #Scaler(y)
    ...
    def train(self):
        ...
        for i, data in enumerate(self.train_loader):
            ...
            output = self.model.forward2(src, frac)
    ...
    def train2(self):
        ...
        for i, data in enumerate(self.train_loader):
            ...
            y = self.scaler.normalizeY(y)   # here we do log10(y+1)  
            y = self.scaler.scaleY(y)       # here we do scaler fit with Standard Scaler 
             
            src, frac = X.squeeze(-1).chunk(2, dim=1)
            
            #print("before: ", src,frac)
            ...
            output = self.model.forward2(src, frac)
            prediction = output #uncertainty = output.chunk(2, dim=-1)
            #print("train")
            loss = self.criterion(y.view(-1), prediction.view(-1))
            
                             
            
             
            
            #loss.requires_grad = True # Because of RuntimeError: element 0 of tensors does not require grad and does not have a grad_fn
            ...
            if self.stepping:
                self.lr_scheduler.step()
            #with torch.no_grad():
            #    y = self.scaler.unscaleY(y)
            #    y = self.scaler.unnormalizeY(y)
            #    
            #    prediction = self.scaler.unscaleY(prediction)
            #    
            #    prediction = self.scaler.unnormalizeY(prediction)    
            ...
            if learning_time:
                ...
                #print("in train : act_v =", act_v[0],"pred_v =", pred_v[0])    
                mae_v = mean_absolute_error(act_v, pred_v)
    ...
    def predict2(self, loader):
        ...
        with torch.no_grad():
            for i, data in enumerate(loader):
                ...
                prediction  = output#.chunk(2, dim=-1)
                ...
                #print("actual data:", act[data_loc], "predicted:", pred[data_loc])
