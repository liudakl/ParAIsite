...
class MEGNet(nn.Module, IOMixIn):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        #print("dim_blocks_in:",dim_blocks_in)
        #print("dim_blocks_out:",dim_blocks_out)
        block_args = {
...
class MEGNet_changed(nn.Module, IOMixIn):
    """DGL implementation of MEGNet."""
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.output_proj = MLP_last_layer(
        # S2S cats q_star to output producing double the dim
            dims=[2 * 2 * dim_blocks_out + dim_blocks_out, *hidden_layer_sizes_output],
            activation=activation,
            activate_last=False,
        )
    ...
    def forward(self, g: dgl.DGLGraph, state_attr: torch.Tensor | None = None, **kwargs):
        ...
        output = self.output_proj(vec)
        output = vec
        print("output in CHANGED: ",output)
        if self.is_classification:
            output = torch.sigmoid(output)
